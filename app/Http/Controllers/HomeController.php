   ...
   /* public function index()
    ...
    public function registerProduct(Request $request, string $locale)
    {
        ...
        Product::create([
            ...
            'name' => $name,
            'price' => doubleval($request->get('unit_price')),
            'others' => ''
        ]);

        $msg = __("Produit enregistré avec succès");
        session()->flash('status', $msg);
        return back()->with('status', $msg);
    }

    public function deleteProduct(Request $request, string $locale, string $productid)
    {
        $product = Product::where('id', $productid)->first();
        if($product === null){
            $msg = __("Produit non reconnu");
            session()->flash('error', $msg);
            return back()->withErrors(['error' => $msg]);
        }

        $product->delete();
        $msg = __("Produit supprimé avec succès");
        session()->flash('status', $msg);
        return back()->with('status', $msg);
    }

    public function updateProduct(Request $request,string $locale, string $productid)
    {
        $validator = Validator::make($request->all(), [
            'name' => 'required|string|max:255|min:2|unique:products,name',
            'unit_price' => 'required|numeric',
        ...
        $msg = __("Produit modifié avec succès");
        session()->flash('status', $msg);
        return back()->with('status', $msg);
    }
